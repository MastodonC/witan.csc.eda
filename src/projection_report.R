...
output_all_charts <- function() {
  ...
  for (test.age in 0:17) {
    ...
    actuals <- bind_rows(actuals, cic_transpose %>% 
                           dplyr::select(date, as.character(test.age)) %>% 
                           mutate(variable = "counts") %>% 
...
plot_summary <- function(project_from, project_yrs) {
  ...
  historic_episodes <- projected_episodes %>%
    ...
    group_by(ID) %>%
    slice(1) %>%
    dplyr::select(Simulation, ID, Period.Start, Period.End, Admission.Age, Birthday, Provenance)
...
all_episodes <- projected_episodes %>%
  ...
  slice(1) %>%
  ungroup %>%
  dplyr::select(Simulation, Provenance, ID, Period.Start, Period.End, Admission.Age, Birthday, Provenance) %>%
